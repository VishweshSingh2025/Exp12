...
read_csv() – Loads categorical data from a CSV file into a DataFrame.

value_counts() – Counts the frequency of each unique category value.

value_counts(normalize=True) – Returns percentage distribution of category values.

crosstab() – Creates a cross-tabulation table to compare two categorical variables.

groupby() – Groups dataset rows based on a selected categorical column.

unique() – Returns all unique values present in a column.

sort_values() – Sorts the dataset based on selected categorical values.

filtering using condition – Extracts rows that satisfy a specific category condition.

sum() – Adds values, often used after percentage or count operations.
